...
typedef SWIFT_ENUM(NSInteger, GSTrackingOptions, closed) {
  GSTrackingOptionsNone = 1,
  GSTrackingOptionsPowerSaver = 2,
  GSTrackingOptionsBalanced = 3,
  GSTrackingOptionsHighPerformance = 4,
};
...
+ (void)intialize:(NSString * _Nonnull)publishKey :(NSString * _Nullable)urlString;
...
+ (void)startTracking:(enum GSTrackingOptions)trackingOption;
+ (void)stopTracking;
...
+ (void)notificationOpenedHandler:(UNNotificationResponse * _Nonnull)resposne SWIFT_AVAILABILITY(ios,introduced=10.0);
